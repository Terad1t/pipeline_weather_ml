...
def create_dataframe(path_name:str) -> pd.DataFrame:
    ...
    logging.info("Criando DF do arquivo JSON...")
    path = path_name

    if not path.exists():
        raise FileNotFoundError(f"Arquivo not found: {path}")
...
def normalize_weather_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logging.info(f"Coluna 'weather' normalizada com {len(df.columns)} colunas")
...
def data_transformations():
    ...
    df = rename_columns(df, columns_names_to_renam)
